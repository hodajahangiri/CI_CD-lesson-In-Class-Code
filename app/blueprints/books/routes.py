...
# @cache.cached(timeout=30) #If you cache paginated routes it will cache a single page
def get_books():
    try:
        page = int(request.args.get('page'))
        per_page = int(request.args.get('per_page'))
        query = select(Books)
        books = db.paginate(query,page=page, per_page=per_page) #Handles our pagination so we don't have to be worry about pagination with hard code
...
def get_popular_books():
    ...
    # popular_books = db.session.query(Books).order_by(Books.times_borrowed.desc()).limit(5).all()

    books = db.session.query(Books).all()
    # Sort books list based off of how many loans the have been apart of
    ...
    # return books_schema.jsonify(books[:5])
    # return books_schema.jsonify(popular_books)


@books_bp.route('/search', methods=["GET"])
def search_books():
    title = request.args.get('title') # Accessing the query parameters from my url
